...
class HypreCmake(CMakePackage, CudaPackage):
    ...
    def cmake_args(self):
        ...
        args = [
            ...
            from_variant("HYPRE_WITH_DSUPERLU", "superlu-dist"),
        ...
        if "+umpire" in spec:
            if "~cuda" in spec:
                args.append(define("HYPRE_WITH_UMPIRE_HOST", True))
            else:
                args.append(define("HYPRE_WITH_UMPIRE", True))

            args.append(define("TPL_UMPIRE_LIBRARIES", spec["umpire"].libs))
            args.append(define("TPL_UMPIRE_INCLUDE_DIRS", spec["umpire"].headers.directories))
